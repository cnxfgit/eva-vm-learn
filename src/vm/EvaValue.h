...
enum class ObjectType { STRING, CODE, NATIVE, FUNCTION };
...
#define NUMBER(value)                         \
    EvaValue {                                \
        EvaValueType::NUMBER, .number = value \
    }

#define BOOLEAN(value)                          \
    EvaValue {                                  \
        EvaValueType::BOOLEAN, .boolean = value \
    }

#define ALLOC_STRING(value)                                     \
    EvaValue {                                                  \
        EvaValueType::OBJECT, .object = new StringObject(value) \
    }

#define ALLOC_CODE(name, arity)                                     \
    EvaValue {                                                      \
        EvaValueType::OBJECT, .object = new CodeObject(name, arity) \
    }

#define ALLOC_NATIVE(fn, name, arity)                                     \
    EvaValue {                                                            \
        EvaValueType::OBJECT, .object = new NativeObject(fn, name, arity) \
    }

#define ALLOC_FUNCTION(co)                                     \
    EvaValue {                                                 \
        EvaValueType::OBJECT, .object = new FunctionObject(co) \
    }
